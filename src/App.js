      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h3 className={`font-semibold text-gray-900 ${compact ? 'text-sm' : 'text-base'} truncate`}>
            {isSampleData ? connection.name : <span style={{filter: 'blur(6px)'}}>{connection.name}</span>}
          </h3>
                            ...
                            <div className="bg-gray-50 rounded-lg p-4">
                              <pre className="whitespace-pre-wrap text-sm text-gray-800 leading-relaxed font-sans">
                                {isSampleData
                                  ? generatedMessage
                                  : (() => {
                                      // Blur out the contact name in the message
                                      // Replace all occurrences of the contact's name with a blurred span
                                      const name = selectedConnection?.name || '';
                                      if (!name) return generatedMessage;
                                      // Escape RegExp special chars in name
                                      const nameRegex = new RegExp(name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g');
                                      const parts = generatedMessage.split(nameRegex);
                                      const blurredName = `<span style=\"filter: blur(6px); display: inline-block;\">${name}</span>`;
                                      // Interleave parts and blurredName
                                      let result = [];
                                      for (let i = 0; i < parts.length; i++) {
                                        result.push(parts[i]);
                                        if (i < parts.length - 1) result.push(blurredName);
                                      }
                                      // Render as HTML
                  ...
                  <div className="space-y-4">
                    {highValueToShow.map((conn) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-gray-900 truncate">{isSampleData ? conn.name : <span style={{filter: 'blur(6px)'}}>{conn.name}</span>}</p>
